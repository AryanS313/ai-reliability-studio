...
def test_sonnet_manifest_labels_requested_temperature_and_persists_effective_provenance(
    ...
    project_id = repository.create_project(context, {"name": "Sampling fixture"})
    monkeypatch.setattr(database, "get_repository", lambda: repository)
    monkeypatch.setattr(database, "current_context", lambda *args, **kwargs: context)
    payload = anthropic_payload(model="claude-sonnet-5")
    mock_transport(monkeypatch, lambda request: httpx.Response(200, json=payload))
    ...
    row = rows.iloc[0]
    manifest = json.loads(repository.list_runs(context)[0]["manifest_json"])
    assert row["execution_status"] == "passed"
    assert manifest["model"]["temperature"] == temperature
    assert manifest["model"]["seed"] == seed
    assert manifest["model"]["token_limit"] == max_tokens
    assert manifest["model"]["sampling_setting_semantics"] == "requested"
    assert manifest["model"]["effective_sampling_location"] == "per_execution_result.metadata.sampling"
