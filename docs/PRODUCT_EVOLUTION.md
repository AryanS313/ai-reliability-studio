...
This is the first edition of this consolidated history; earlier evidence was distributed across Git, implementation reports and design-partner documents.

- Verified the historical anchors against source revisions and GitHub merge records. The earliest recorded app is **31 May**, not proof of when development or the idea began. July's expansion was authored on 10 July Pacific time and committed on **11 July in India**.
- Corrected the original sample denominator: **five documents, 56 CSV cases and ten navigation destinations**. The legacy 56-row CSV remains in the repository; today's primary journey uses a separate **32-case synthetic dataset**, and the saved-answer example contains **three authored answers**. These are different fixtures, not growth or contraction in customer usage.
- Established that **August already removed provider-to-mock fallback and blocked synthetic launch verdicts**. September strengthened those contracts and public workflows; it did not first invent every evidence safeguard.
- Distinguished August repository/workspace scoping from September per-visitor isolation, and September temporary server storage from subsequent browser-tab execution. No confirmed cross-visitor exposure incident is asserted.
- Reconciled the delivery split: the reviewed September beta is **deployed through Sites**, while **GitHub main remains at the 8 September revision**. GitHub initially rejected the new branch push for missing `workflow` permission. The owner completed the device flow, and the required scope was verified on 16 September. Push, merge and remote CI still need observed completion.
- Added a standing maintenance and reporting contract in [AGENTS.md](../AGENTS.md). This documentation milestone changes reporting discipline, not application behavior or customer evidence.
...
| **Other committed local branches** | `codex/design-partner-release` in the integration worktree; `codex/design-partner-beta` in the requested checkout. Application `b9d8131`; later release-report receipts `3ca82d4`, `b51aaa7` | Current code and verification history are committed locally. The remote release branch was absent at the check; a rejected push did not publish it |
| **This documentation milestone** | `docs/PRODUCT_EVOLUTION.md`, standing repository instructions and documentation links; identify its revision with `git log -1 --format=%H -- docs/PRODUCT_EVOLUTION.md` | Local documentation revision; not part of the verified GitHub main or already-published browser archive |
...
| CLI/CI, SQLite/PostgreSQL repository contracts and role controls | Native repository code, not a managed browser service | Native suite and disposable PostgreSQL pass. Current remote CI has not run for the new release branch yet; authorization is now complete |
...
1. **GitHub release handoff:** the owner completed authorization and the login now has `workflow` scope. Push the complete branch, open the prepared PR, run the Python 3.11/3.12, browser, PostgreSQL and container checks, resolve failures and merge only after observed success. The earlier permission denial remains part of the history; it is no longer the active blocker.
