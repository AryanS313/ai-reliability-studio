...
The temporary primary address remains [the existing Streamlit app](https://ai-reliability-studio.streamlit.app/). The September 16 deployment's sample-only behavior was an agent-imposed restriction, **not owner-approved product scope**. On September 17 the owner explicitly required all customer workflows on this main site, with no terminal or local-install instructions. The native hosted-session correction is now merged and live as application `2026.09.17.1`, with passing merged-main checks and observed custom preparation, consented public HTTPS transport, synthetic review, downloads and independent-session reset. The [hosted review](design-partner/hosted-workflow-review.md) records the remaining real-account and manual-testing limits; the earlier narrower completion claim remains corrected.
