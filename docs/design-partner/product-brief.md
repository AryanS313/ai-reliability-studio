...
**Delivery and acceptance update (17 September 2026):** the owner rejected the agent’s unapproved sample-only/local-install restriction. The correction [merged through PR 13](https://github.com/AryanS313/ai-reliability-studio/pull/13) at `bcdd610`; all five branch/PR/main jobs passed. Native `2026.09.17.1` is live on the primary site, with custom preparation, consented HTTPS transport, synthetic baseline/revision review, download controls and two-session reset observed. Classification: **bounded hosted design-partner beta as engineering delivery; full owner acceptance unproven** pending an approved real-assistant/account trial, independent usability/domain review and manual-picker checks. The separately published browser stays `2026.09.16.1`. Customer/commercial outcomes remain unmeasured. See the [release review](release-review.md) and [product history](../PRODUCT_EVOLUTION.md).
...
**Required delivery experience:** a nontechnical visitor must be able to complete the guided custom journey online on the main site: create a project, provide approved sources and cases, connect an assistant through understandable fields, authorize evaluation, inspect and export evidence, and return to compare a revision. An engineer may supply the assistant's integration details; a repository checkout, local installation or raw JSON configuration must not be required to use the product. This is an owner requirement, not a validated customer outcome. The bounded hosted custom journey is now observed on the main site; real-assistant and independent-user acceptance remain unproven. The Cloudflare decision is paused.

**Implemented online, with explicit remaining acceptance:** the hosted release includes project download/resume for temporary sessions. It preserves stored source passages and versioned inputs, along with explicitly unverified historical results and decisions. Original uploaded binaries are not included; credentials and qualifying calibration do not transfer. Local sample download/restore/review/export and focused automated checks passed, as scoped in the release review. Live project/report download controls and two synthetic-run comparison were observed; downloaded contents and manual picker restore remain unverified. None establishes a customer’s repeat use.

The final local candidate passed **1,205 tests with zero skips** and **87.53% source coverage**, including six PostgreSQL service checks. Branch, PR and merged-main checks passed across both supported Python versions, PostgreSQL, the Linux container and browser build. The [hosted workflow review](hosted-workflow-review.md) records those receipts, the bounded live custom journey and remaining real-account/independent-user/manual-picker acceptance. These engineering counts do not validate the ICP or adoption thesis.
...
4. In native hosted-session mode on the main site, connect the staging assistant through the guided target form and check the connection and credentials; live public HTTPS transport was verified, while approved real-assistant/account acceptance remains open. In the browser, import saved assistant answers for source review, or explicitly connect a supported model provider; arbitrary assistant HTTP is unavailable.
...
**Current delivery boundary:** native hosted release `2026.09.17.1` is merged and live. Custom preparation, a consented public HTTPS check, synthetic baseline/revision comparison, readable evidence/download controls and two-tab reset were observed. This verifies the bounded engineering journey, not real assistant quality or independent owner acceptance. The separately published tab-local browser remains `2026.09.16.1`, with its earlier saved-answer/provider and custody limits. The pilot owner supplies approved data, staging access and an independent reviewer; a synthetic run or non-AI endpoint cannot substitute.
...
**Current evidence limit:** native, actual Wasm and live hosted/browser checks establish only their exercised behavior and bounded controls. Full owner acceptance still needs approved real-assistant/account access, independent first-use comprehension/domain calibration, manual downloaded-file/picker verification and accessibility assessment. Adoption, recurring value and willingness to continue require actual design partners. See [discovery plan](discovery-plan.md) and [success metrics](success-metrics.md).
