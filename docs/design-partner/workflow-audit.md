...
**17 September delivery update:** the owner-rejected sample-only/local-install restriction is superseded. [PR 13](https://github.com/AryanS313/ai-reliability-studio/pull/13) merged at `bcdd610`; all five branch/PR/main jobs passed. Native `2026.09.17.1` is live, with custom preparation, consented HTTPS transport, two synthetic evaluations and inconclusive revision comparison, download controls and two-tab reset observed. Final local checks: **1,205 passed, zero skips, 87.53% coverage**. This is a bounded hosted beta as engineering delivery; full owner acceptance remains unproven without approved real-assistant access, independent usability/domain review and manual-picker verification. No customer/commercial outcome is measured. Exact receipts are in the [hosted workflow review](hosted-workflow-review.md); dated baselines below remain history.
